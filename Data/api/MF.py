...
def load_dataframes():
    ...
    # con.close()
    db_connection_str = 'mysql+pymysql://hongju:hongju1004@k02b1051.p.ssafy.io/'
...
def set_algo(user_id,area=None):
    ...
    try:
        ...
        # 전국 검색
        if len(area) ==0 or area is None:
            iids = raw['store_id'].unique()
        else:

            tmp = df[df['addr'].str.contains('|'.join(area))]
        ...
        i_max = pred_ratings.argsort()[::-1][:3]
