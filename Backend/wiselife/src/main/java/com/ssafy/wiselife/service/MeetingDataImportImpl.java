...
public class MeetingDataImportImpl implements IMeetingDataImport {
	...
	public static int findCategory(String title) {
		...
		category.add("레저 스포츠 축구 등산 요가 골프 카누 수상레저 필라테스 클라이밍 보드 야구 농구 축구 수영 호신술 다이어트 자세교정 암벽등반 보드 스케이트 롤러스케이트 헬스 다이빙 패들보드 승마 탁구 윈드서핑 복싱 서핑 당구");
		category.add("요리 쿠킹 베이킹 커피 라떼 맛있는 마카롱 전통주 전통다과 스콘 바람떡 케이크 맥주 와인 바리스타 위스키 한식 중식 갸토 흑임자 양식 일식 크림치즈 타르트 음식 누텔라 핸드드립 참기름 뷰티차 볶바인딩 오븐 찰빵");
		category.add("수공예 공방 가죽공예 목공예 금속공예 공예 꽃꽂이 네트백 장신구 키링 입욕제 석고 컵 카네이션 스테인드글라스 향초 방향제 크로스백 도자기 의류 가죽 도예 꽃다발 플레이트 우드조명 무드등 향수 우드스피커 우드 샴푸 인테리어 소품 비누 플라워 프랑스자수 자수 그릇 협탁 천연 천연비누 꽃바구니 라탄 캔들 아기옷 공예 원목 굿즈 디퓨저 제작 수제 귀걸이 팔찌 목걸이 원목도마");
		category.add("놀이 게임 방탈출 보드게임 pc방 오락 조이스틱");
		category.add("문화 영화 책 공연 전시 콘서트 뮤지컬");
		category.add("예술 음악 미술 사진 영상 댄스 연주 데코 작사 훌라 KPOP 촬영 카메라 보컬 피아노 아크릴화 수채화 포토샵 풍경화 그림 아트클래스 드로잉 인물화 노래 기초발성 바이올린 그리기 가야금 드럼 스케치 일렉 색연필화");
