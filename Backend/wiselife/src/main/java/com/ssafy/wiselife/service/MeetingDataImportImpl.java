...
public class MeetingDataImportImpl implements IMeetingDataImport {
	...
	public static int findCategory(String title) {
		...
		category.add("요리 쿠킹 베이킹 커피 라떼 맛있는 마카롱 전통주 전통다과 스콘 바람떡 맥주 와인 바리스타 위스키 한식 중식 갸토 흑임자 양식 일식 크림치즈 타르트 음식 누텔라 핸드드립 참기름 뷰티차 볶바인딩 오븐 찰빵");
		category.add("수공예 공방 가죽공예 목공예 금속공예 공예 꽃꽂이 네트백 장신구 키링 입욕제 석고 스테인드글라스 향초 방향제 크로스백 도자기 의류 가죽 도예 꽃다발 플레이트 우드조명 무드등 향수 우드스피커 우드 샴푸 인테리어 소품 비누 플라워 프랑스자수 자수 그릇 협탁 천연 천연비누 꽃바구니 라탄 캔들 아기옷 공예 원목 굿즈 디퓨저 제작 수제 귀걸이 팔찌 목걸이 원목도마");
